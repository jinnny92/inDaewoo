...
<div class="jumbotron">
<h1 class="text-center">게시글 검색 목록</h1>
</div>
<table class="table table-striped">
  ...
  <c:forEach items="${list}" var="item">
    <tr>
      <td>${item.bno}</td>
      <td><a href="/board/read/${item.bno}">${item.title}</a></td>
      <td>${item.writer}</td>
...
<nav aria-label="...">
  <ul class="pagination">
    <li class="page-item">
      <a class="page-link" href="/board/search?curpage=${pt.curPage <= 1? 1: pt.curPage-1}&criteria=${criteria}&keyword=${keyword}">Previous</a>
    </li>
    
    <c:forEach begin="${pt.beginPageNum}" end="${pt.finishPageNum}" var="page">
    	<c:if test="${page == pt.curPage}">
    		 <li class="page-item active" aria-current="page">
      			<a class="page-link" href="/board/search?curpage=${pt.curPage}&criteria=${criteria}&keyword=${keyword}">${page}</a>
    		</li>
    	</c:if>
    	
    	<c:if test="${page != pt.curPage}">
    		  <li class="page-item">
    		  <a class="page-link" href="/board/search?curpage=${page}&criteria=${criteria}&keyword=${keyword}">${page}</a></li>
    	</c:if>
    ...
    <li class="page-item">
      <a class="page-link" href="/board/search?curpage=${pt.curPage >= pt.totalPage? pt.totalPage : pt.curPage+1}&criteria=${criteria}&keyword=${keyword}">Next</a>
    </li>
  </ul>
</nav>
